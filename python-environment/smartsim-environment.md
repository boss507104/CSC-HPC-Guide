...
SmartRedis installs from the CSC-maintained `v1.0.0-csc` release, while SmartSim installs from `v1.0.1-csc`. These releases already contain:
...
```text
Python        3.12
SmartSim      1.0.1+csc (PentagonToy/SmartSim @ v1.0.1-csc)
...
Choose target architecture + PySR/Julia toggle (Section 1)
  ...
  |     --> install SmartRedis v1.0.0-csc + SmartSim v1.0.1-csc
...
| SmartSim | `1.0.1+csc` (`PentagonToy/SmartSim @ v1.0.1-csc`) | Orchestration; Redis, RedisAI, and JAX worker lifecycle |
...
# SmartRedis v1.0.0-csc and SmartSim v1.0.1-csc include the CSC
...
**GitHub release installation fails** — confirm outbound network access from the build node and verify that the SmartRedis `v1.0.0-csc` and SmartSim `v1.0.1-csc` tags are accessible.
...
* SmartSim and SmartRedis install from the CSC releases (`PentagonToy/SmartSim @ v1.0.1-csc`, `PentagonToy/SmartRedis @ v1.0.0-csc`), not PyPI — no runtime patching remains in `extra4SmartSim.sh` / `update4SmartSim.sh`, and neither is affected by the PySR toggle.
